...
function redConverter() {
  ...
  // Recorremos los píxeles y establecemos el componente verde (G) y azul (B) a 0
...
function greenConverter() {
  ...
  // Recorremos los píxeles y establecemos el componente rojo (R) y azul (B) a 0
  for (let i = 0; i < pixels.length; i++) {
    for (let j = 0; j < pixels[i].length; j++) {
      pixels[i][j][0] = 0; // Canal G a 0
      pixels[i][j][2] = 0; // Canal B a 0
    }
  }
...
function blueConverter() {
  ...
  // Recorremos los píxeles y establecemos el componente rojo (R) y verde (G) a 0
  for (let i = 0; i < pixels.length; i++) {
    for (let j = 0; j < pixels[i].length; j++) {
      pixels[i][j][0] = 0; // Canal G a 0
      pixels[i][j][1] = 0; // Canal B a 0
    }
  }
...
function greyConverter() {
  ...
  for (let i = 0; i < pixels.length; i++) {
    for (let j = 0; j < pixels[i].length; j++) {
      let average = Math.round((pixels[i][j][0] + pixels[i][j][1] + pixels[i][j][2]) / 3);
...
function dimBrightness(dimFactor) {
  ...
  for (let i = 0; i < pixels.length; i++) {
    for (let j = 0; j < pixels[i].length; j++) {
      pixels[i][j][0] /= dimFactor; // Canal R
      pixels[i][j][1] /= dimFactor; // Canal G
      pixels[i][j][2] /= dimFactor; // Canal B
    }
  }
...
function merge(alphaFirst, alphaSecond) {
  ...
  // NOTA: Las imagenes tendrán que tener la misma dimension
  for (let i = 0; i < catPixels.length; i++) {
    let newRow = [];
    for (let j = 0; j < catPixels[i].length; j++) {
      let mergedPixel = [
        alphaFirst * catPixels[i][j][0] + alphaSecond * dogPixels[i][j][0], // Canal R
        alphaFirst * catPixels[i][j][1] + alphaSecond * dogPixels[i][j][1], // Canal G
        alphaFirst * catPixels[i][j][2] + alphaSecond * dogPixels[i][j][2]  // Canal B
      ];
