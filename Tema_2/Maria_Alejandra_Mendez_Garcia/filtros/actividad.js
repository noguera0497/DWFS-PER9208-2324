...
function greyConverter() {
  ...
  for(let i=0; i<pixels.length; i++)
  {
    for(let j = 0; j < pixels[i].length; j++)
    {
      ...
      g = pixels[i][j][1];
      b = pixels[i][j][2];
      media_pixel=Math.round((r+g+b)/3)
       pixels[i][j]=[media_pixel,media_pixel,media_pixel]
    }
  }
  handler.savePixels(pixels, outputPath);
}
...
function scaleDown() {
  ...
  //Aqui tu codigo
  let imagen_escala_down = [];
  for (let i = 0; i < pixels.length; i=i+2) {
    let nuevospixeles = [];
    for (let j = 0; j < pixels[i].length; j=j+2) {
      nuevospixeles.push(pixels[i][j])
    }
    imagen_escala_down.push(nuevospixeles)
  }


  handler.savePixels(imagen_escala_down, outputPath, handler.getShape()[0] / 2, handler.getShape()[1] / 2);
}
...
function invertColors() {
  ...
  for(let i=0; i<pixels.length; i++)
  {
    for(let j = 0; j < pixels[0].length; j++)
    {
      ...
      nuevog=255-g;
      nuevob=255-b;
      pixels[i][j]=[nuevor, nuevog,nuevob]
    }
  }
  handler.savePixels(pixels, outputPath);
}
...
function merge(alphaFirst, alphaSecond) {
  ...
  for(let i=0; i<catPixels.length; i++)
  {
    ...
    {
      let nuevopixel=[]
      r= (catPixels[i][j][0]*alphaSecond)+(dogPixels[i][j][0]*alphaFirst);
      g = (catPixels[i][j][1]*alphaSecond)+(dogPixels[i][j][1]*alphaFirst);
      b = (catPixels[i][j][2]*alphaSecond)+(dogPixels[i][j][2]*alphaFirst);
      fusionpixeles=[r,g,b]
      nuevos_pixeles.push(fusionpixeles)
    }
    pixels.push(nuevos_pixeles)
  }
