...
function redConverter() {
    ...
    // Recorre cada píxel y establece los canales G y B a 0 para convertirlo en escala de rojos
    for (let i = 0; i < pixels.length; i++) {
        for (let j = 0; j < pixels[i].length; j++) {
            pixels[i][j] = [pixels[i][j][0], 0, 0]; // Establece el canal verde (G) a 0 y el canal azul a 0
        }
    }  
...
function greenConverter() {
    ...
    // Recorre cada píxel y establece los canales R y B a 0 para convertirlo en escala de verdes
...
function blueConverter() {
    ...
    // Recorre cada píxel y establece los canales R y G a 0 para convertirlo en escala de azules
    for (let i = 0; i < pixels.length; i++) {
        for (let j = 0; j < pixels[i].length; j++) {
            pixels[i][j] = [0, 0, pixels[i][j][1]]
        }
    }
    handler.savePixels(pixels, outputPath);
}
...
function greyConverter() {
    ...
    // Recorre cada píxel y calcula la media de los valores RGB
    for (let i = 0 ; i < pixels.length ; i++){
        for (let j = 0; j < pixels[i].length; j++) {
          const media = (pixels[i][j][0] + pixels[i][j][1] + pixels[i][j][2])/3
          pixels[i][j][0] = media
          pixels[i][j][1] = media
          pixels[i][j][2] = media
        }
...
function blackAndWhiteConverter() {
    ...
    for (let i = 0 ; i < pixels.length ; i++){
        for (let j = 0; j < pixels[i].length; j++) {
          const mediaPixels = (pixels[i][j][0] + pixels[i][j][1] + pixels[i][j][2])/3
          if (mediaPixels < 128) {
            pixels[i][j][0] = 0
            pixels[i][j][1] = 0
            pixels[i][j][2] = 0
...
function scaleDown() {
    ...
    // Crea una nueva imagen utilizando solo los valores de las filas y columnas pares
    let scaledPixels = [];
    for (let i = 0; i < pixels.length; i += 2) {
        let scaledRow = [];
        for (let j = 0; j < pixels[i].length; j += 2) {
            scaledRow.push(pixels[i][j]);
        }
        scaledPixels.push(scaledRow);
    }
...
function dimBrightness(dimFactor) {
    ...
    // Reduce el brillo de la imagen según el factor de atenuación
    for(let i = 0; i < pixels.length;i++) {
      for(let j = 0; j < pixels[i].length;j++) {
...
function merge(alphaFirst, alphaSecond) {
  ...
  for (let row = 0; row < arrayRows; row++) {
    let newRow = [];    
    let arrayCols = catPixels[row].length > dogPixels[row].length ? dogPixels[row].length : catPixels[row].length;
    for (let column = 0; column < arrayCols; column++) {      
