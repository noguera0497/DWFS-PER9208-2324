...
function redConverter() {
  ...
  //Aqui tu codigo
  for (const row_pixel of pixels) {
    for (const pix of row_pixel) {
      // R G B
      pix[1] = 0;
      pix[2] = 0;
    }
...
function greyConverter() {
  ...
  //Aqui tu codigo
  // aplicar formula general: Y=0.299×R+0.587×G+0.114×B
  for (const row_pixel of pixels) {
    for (const pix of row_pixel) {
      // R G B
      let y = 0.299 * pix[0] + 0.587 * pix[1] + 0.114 * pix[2]
      pix[0] = y;
      pix[1] = y;
      pix[2] = y;
    }
...
function scaleDown() {
  ...
  handler.savePixels(
    pixels
      ...
      .map((row_pixel) => row_pixel.filter((_, index) => index % 2 === 0)),
    outputPath,
...
function dimBrightness(dimFactor) {
  ...
  for (const row_pixel of pixels) {
    for (const pix of row_pixel) {
      // R G B
      pix[0] = pix[0] / dimFactor;
      pix[1] = pix[1] / dimFactor;
      pix[2] = pix[2] / dimFactor;
    }
  }
...
function merge(alphaFirst, alphaSecond) {
  ...
  for (const i in catPixels) {
    const row_pixels = catPixels[i];
    pixels[i] = [];
    for (const j in row_pixels) {
      pixels[i][j] = [];
      const dogPixel = dogPixels[i][j];
      const catPixel = row_pixels[j];
      const p = pixels[i][j];
      p[0] = catPixel[0] * alphaSecond + dogPixel[0] * alphaFirst;
      p[1] = catPixel[1] * alphaSecond + dogPixel[1] * alphaFirst;
      p[2] = catPixel[2] * alphaSecond + dogPixel[2] * alphaFirst;
    }
  }
