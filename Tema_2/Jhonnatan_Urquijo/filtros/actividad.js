...
function greyConverter() {
  ...
  pixels.forEach(row => {
    row.forEach(pixel => {
      const suma = pixel.reduce((accumulator, element) => accumulator + element, 0);
      const media = suma / pixel.length;
      pixel.fill(media);
    });
  });
...
function blackAndWhiteConverter() {
  ...
  pixels.forEach(row => {
    row.forEach(pixel => {
      const suma = pixel.reduce((accumulator, element) => accumulator + element, 0);
      const media = suma/pixel.length;
      media < 128 ? pixel.fill(0) : pixel.fill(255);
    });
  });
...
function scaleDown() {
  ...
  let cantRows = pixels.length;
  let columns = pixels[0].length;
  let tempPixels = [];

  for (let i = 0; i < cantRows; i+=2) {
    let tempRow = [];
    for (let j = 0; j < columns; j+=2) {
      tempRow.push(pixels[i][j]);
    }
...
function dimBrightness(dimFactor) {
  ...
  pixels.forEach(row => {
    row.forEach(pixel => {
      pixel.forEach((element, index) => {
        pixel[index] = element / dimFactor;
      });
    });
  });
...
function merge(alphaFirst, alphaSecond) {
  ...
  let cantRows = catPixels.length;
  let cantColumns = catPixels[0].length;
