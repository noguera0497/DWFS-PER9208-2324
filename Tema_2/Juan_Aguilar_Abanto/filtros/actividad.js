...
function redConverter() {
  ...
  for (let i = 0; i < pixels.length; i++) {
    for (let j = 0; j < pixels[i].length; j++) {
      // Verificar si el píxel tiene los canales R, G y B
      if (pixels[i][j].length >= 3) {
        // Poner el canal G (verde) y B (azul) a 0
...
function greyConverter() {
  ...
  for(let i = 0; i < pixels.length;i++)
  {
    for(let j = 0; j < pixels[i].length;j++)
    {
      if (pixels[i][j].length >= 3)
      {
        var media = (pixels[i][j][0] + pixels[i][j][1] +  pixels[i][j][2]) / 3;
...
function merge(alphaFirst, alphaSecond) {
  ...
  for(let i = 0; i < catPixels.length;i++)
  {
    ...
    {
      if (catPixels[i][j].length >= 3)
      {
        let rgb1 =  (dogPixels[i][j][0] * alphaFirst) + (catPixels[i][j][0] * alphaSecond);

        let rgb2 =  (dogPixels[i][j][1] * alphaFirst) + (catPixels[i][j][1] * alphaSecond);

        let rgb3 =  (dogPixels[i][j][2] * alphaFirst) + (catPixels[i][j][2] * alphaSecond);

        let pixel = [rgb1,rgb2,rgb3]
