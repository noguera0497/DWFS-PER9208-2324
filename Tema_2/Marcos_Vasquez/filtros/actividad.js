...
function redConverter() {
  ...
  for (let index = 0; index < pixels.length; index++) {
    const filas = pixels[index];
    for (let index2 = 0; index2 < filas.length; index2++) {
      const pixel = filas[index2];
      pixel[1] = 0
      pixel[2] = 0
    }
  }
...
function greyConverter() {
  ...
  for (let index = 0; index < pixels.length; index++) {
    ...
    for (let index2 = 0; index2 < filas.length; index2++) {
      const pixel = filas[index2];
      const media = pixel.reduce((a, b) => a + b, 0)/pixel.length
      pixel[0] = Math.trunc(media)
      pixel[1] = Math.trunc(media)
      pixel[2] = Math.trunc(media)
    }
  }
...
function blackAndWhiteConverter() {
  ...
  for (let index = 0; index < pixels.length; index++) {
    ...
    for (let index2 = 0; index2 < filas.length; index2++) {
      const pixel = filas[index2];
      const media = pixel.reduce((a, b) => a + b, 0)/pixel.length
      pixel[0] = media < 128 ? 0 : 255
      pixel[1] = media < 128 ? 0 : 255
      pixel[2] = media < 128 ? 0 : 255
    }
  }
...
function merge(alphaFirst, alphaSecond) {
  ...
  //Aqui tu codigo
  for (let index = 0; index < catPixels.length; index++) {
    const filas = catPixels[index];
    let newRow = []
    for (let index2 = 0; index2 < filas.length; index2++) {
      const pixel = filas[index2];
      let pixelAux = []
      pixelAux.push(catPixels[index][index2][0]*alphaFirst + dogPixels[index][index2][0]*alphaSecond)
      pixelAux.push(catPixels[index][index2][1]*alphaFirst + dogPixels[index][index2][1]*alphaSecond)
      pixelAux.push(catPixels[index][index2][2]*alphaFirst + dogPixels[index][index2][2]*alphaSecond)
      newRow.push(pixelAux)
    }
    pixels.push(newRow)
  }
