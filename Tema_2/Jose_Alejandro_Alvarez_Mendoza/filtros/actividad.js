...
function redConverter() {
  ...
  pixels.forEach((pixelsRow, i) => {
    pixelsRow.forEach((pixel, j) => {
...
function greyConverter() {
  ...
  pixels.forEach((pixelsRow, i) => {
    pixelsRow.forEach((pixel, j) => {
      let sum = pixel.reduce((prev, curr) => curr + prev);
      let media = sum/pixel.length;
...
function blackAndWhiteConverter() {
  ...
  pixels.forEach((pixelsRow, i) => {
    pixelsRow.forEach((pixel, j) => {
      let sum = pixel.reduce((prev, curr) => curr + prev);
      let media = sum/pixel.length;
      pixel = (media < 128) ? [0, 0, 0] : [255, 255, 255];
      pixels[i][j] = pixel;
    });
  });
...
function merge(alphaFirst, alphaSecond) {
  ...
  catPixels.forEach((catPixelsRow, i) => {
    pixels[i] = catPixels[i];
    catPixelsRow.forEach((catPixel, j) => {
      pixels[i][j][0] = (alphaFirst * dogPixels[i][j][0]) + (alphaSecond * catPixels[i][j][0]);
      pixels[i][j][1] = (alphaFirst * dogPixels[i][j][1]) + (alphaSecond * catPixels[i][j][1]);
      pixels[i][j][2] = (alphaFirst * dogPixels[i][j][2]) + (alphaSecond * catPixels[i][j][2]);
    });
  });
