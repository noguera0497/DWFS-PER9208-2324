...
function greyConverter() {
  ...
  for (x = 0; x < pixels.length; x++) {
    for (y = 0; y < pixels[x].length; y++) {
        let media = (pixels[x][y].reduce((previous, current) => current += previous)) / 3
...
function scaleDown() {
  ...
  for (let x = 0; x < pixels.length; x += 2) {
    let newRow = [];
    for (let y = 0; y < pixels[x].length; y += 2) {
      newRow.push(pixels[x][y])
    }
    halfImage.push(newRow)
  }
