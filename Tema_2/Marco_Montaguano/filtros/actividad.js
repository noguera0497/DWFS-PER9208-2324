...
function cambiaRGB(pixels, option, dimFactor, alpha) {
    ...
    for (let j = 0; j < pixels[i].length; j++) {
      switch (option) {
        ...
        case 4:
          ...
          break;
        case 5:
          let mediaG = (pixels[i][j][0]+pixels[i][j][1]+pixels[i][j][2])/3;
          if (mediaG < 128){
            pixels[i][j]=[0,0,0]
          } else {
        ...
        case 8:
          ...
          break;
        case 9:
          pixels[i][j][0] = pixels[i][j][0]*alpha;
          pixels[i][j][1] = pixels[i][j][1]*alpha;
          pixels[i][j][2] = pixels[i][j][2]*alpha;
        break;
      }

    }
  }
  console.log("Culmino proceso!!1")
}

function cambiaRGBScale(pixels){
  let newScale = pixels.filter((val, i) => i%2!==0);
  let finalScale = [];
  newScale.forEach(f => {
    finalScale.push(f.filter((val, i) => i%2!==0))
  });
  return finalScale;
}

function cambiaRGBMerge(pixelsFirst, pixelsSecond, alphaFirst, alphaSecond){
  cambiaRGB(pixelsFirst,9,null,alphaFirst);
  cambiaRGB(pixelsSecond,9,null,alphaSecond)
  pixelsFirst.forEach((val,i) =>{
    val.forEach((valCol,j)=>{
      pixelsSecond[i][j][0] = pixelsSecond[i][j][0]+valCol[0];
      pixelsSecond[i][j][1] = pixelsSecond[i][j][1]+valCol[1];
      pixelsSecond[i][j][2] = pixelsSecond[i][j][2]+valCol[2];
    })
  })
  return pixelsSecond;
}
...
function merge(alphaFirst, alphaSecond) {
  ...
  pixels = cambiaRGBMerge(dogPixels,catPixels,alphaFirst,alphaSecond)
