...
function greyConverter() {
    ...
    for (let i = 0; i < pixels.length; i++) {
        for (let j = 0; j < pixels[i].length; j++) {
            const pixel = pixels[i][j];
            // const media = (pixel[0] + pixel[1] + pixel[2]) / 3;
            const media = (pixel.reduce((acc, rgb) => acc + rgb, 0)) / 3;
            // pixel[0] = media;
            // pixel[1] = media;
            // pixel[2] = media;
            pixel.forEach((rgb, index) => pixel[index] = media);
        }
    }
...
function blackAndWhiteConverter() {
    ...
    for (let i = 0; i < pixels.length; i++) {
        for (let j = 0; j < pixels[i].length; j++) {
            const pixel = pixels[i][j];
            const media = (pixel.reduce((acc, rgb) => acc + rgb, 0)) / 3;
            const rgbValue = media >= 128 ? 255 : 0;
            pixel.forEach((rgb, index) => pixel[index] = rgbValue);
        }
    }
...
function dimBrightness(dimFactor) {
    ...
    for (let i = 0; i < pixels.length; i++) {
        for (let j = 0; j < pixels[i].length; j++) {
            const pixel = pixels[i][j];
            pixel.forEach((rgb, index) => pixel[index] = Math.floor(rgb / dimFactor));
        }
    }
...
function invertColors() {
    ...
    for (let i = 0; i < pixels.length; i++) {
        for (let j = 0; j < pixels[i].length; j++) {
            const pixel = pixels[i][j];
            pixel.forEach((rgb, index) => pixel[index] = 255 - rgb);
        }
    }
...
function merge(alphaFirst, alphaSecond) {
    ...
    for (let i = 0; i < catPixels.length; i++) {
        ...
        for (let j = 0; j < catPixels[i].length; j++) {
            ...
            // const newRGB = [];
            // for (let k = 0; k < catPixel.length; k++) {
            //     newRGB.push(Math.floor(catPixel[k] * alphaFirst + dogPixel[k] * alphaSecond));
            // }
            // fila.push(newRGB);

            const newRGB = catPixel.map((rgb, index) => Math.floor(rgb * alphaFirst + dogPixel[index] * alphaSecond));
            fila.push(newRGB);
        }
        pixels.push(fila);
    }
 ...
 */
for (let optionN = 0; optionN <= 9; optionN++) {
    switch (optionN) {
